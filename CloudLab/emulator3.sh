...
ifacetoC=$(ifconfig | grep -B1 "inet 192.168.50.1" | head -n1 | cut -f1 -d:)
ifacetoS=$(ifconfig | grep -B1 "inet 192.168.5.1" | head -n1 | cut -f1 -d:)
...
sudo route add -net 192.168.6.0/24 gw 192.168.5.2
